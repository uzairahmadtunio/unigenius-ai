-- Recreate leaderboard function with SECURITY DEFINER so it can read all profiles
-- (RLS on profiles otherwise restricts the inner SELECT to the caller's own row,
-- which is why users only saw themselves on the leaderboard).
-- Only non-sensitive columns are returned. Limit raised to 100.

CREATE OR REPLACE FUNCTION public.get_leaderboard_filtered(
  time_filter text DEFAULT 'all'::text,
  semester_filter integer DEFAULT NULL::integer
)
RETURNS TABLE(
  user_id uuid,
  display_name text,
...
BEGIN
  ...
  SELECT
    ...
    p.university,
    p.current_semester,
    COALESCE(ds.current_streak, 0)::INTEGER AS streak_days,
